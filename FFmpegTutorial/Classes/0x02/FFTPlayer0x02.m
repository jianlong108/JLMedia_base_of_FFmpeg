...
@interface  FFTPlayer0x02 ()<FFTDecoder0x01Delegate>
{
    //音频流解码器
    FFTDecoder0x01 *_audioDecoder;
    //视频流解码器
    FFTDecoder0x01 *_videoDecoder;
...
{
    FFTPlayer0x02 *player = (__bridge FFTPlayer0x02 *)ctx;
...
- (FFTPlayer0x02 *)openStreamComponent:(AVFormatContext *)ic streamIdx:(int)idx
{
    FFTDecoder0x01 *decoder = [FFTDecoder0x01 new];
    decoder.ic = ic;
...
- (void)readPacketLoop:(AVFormatContext *)formatCtx
{
    ...
    for (;;) {
        ...
        if (ret < 0) {
            ...
            //释放内存
            av_packet_unref(pkt);
            if ([self.delegate respondsToSelector:@selector(player:whenReadPacket:videoPacketCount:)]) {
...
- (void)decoder:(FFTDecoder0x01 *)decoder reveivedAFrame:(AVFrame *)frame
{
    ...
    if ([self.delegate respondsToSelector:@selector(player:whenDecodeFrame:videoFrameCount:)]) {
        [self.delegate player:self whenDecodeFrame:self.audioFrameCount videoFrameCount:self.videoFrameCount];
    }
}
